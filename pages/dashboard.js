...
import { Box, Container, Heading, Text, Stack, Button, HStack, Spinner, useColorModeValue } from "@chakra-ui/react";
...
export default function DashboardPage() {
  ...
  const { isAuthenticated, isLoading, account, company, logout, refreshMe } = useAuth();
  ...
  const testOnlineReport = async () => {
    ...
    try {
      ...
      setReportOutput({ status: null, ok: false, body: { message: error.message } });
    } finally {
  ...
  return (
    <Box bg={pageBackground} minH="60vh" py={{ base: 8, md: 12 }}>
      ...
      <Container maxW="container.lg">
        <HStack justify="space-between" align="flex-start" mb={8} flexWrap="wrap" spacing={4}>
          <Stack spacing={1}>
        ...
        <Stack spacing={6}>
          {reportOutput ? (
            <Box>
              <Text mb={2} fontWeight="600" color={reportOutput.ok ? "green.500" : "red.500"}>
                Report response{reportOutput.status ? ` (HTTP ${reportOutput.status})` : ""}
              </Text>
              <Box
                as="pre"
                bg={outputBackground}
          ...
          <WhatsAppConnectCard account={account} onConnected={() => refreshMe()} />
          <MessagesLogCard />
        </Stack>
      </Container>
    </Box>
